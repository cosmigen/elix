...
const debugLog = path.join(os.tmpdir(), 'elix-shell-debug.log');
try { fs.appendFileSync(debugLog, `\n[${new Date().toISOString()}] START pid=${process.pid} argv=${JSON.stringify(process.argv)}\n`); } catch(e){}
process.on('uncaughtException', (err) => { try { fs.appendFileSync(debugLog, `[UNCAUGHT] ${err.stack || err}\n`); } catch(e){} });
process.on('unhandledRejection', (err) => { try { fs.appendFileSync(debugLog, `[REJECTION] ${err.stack || err}\n`); } catch(e){} });
app.on('will-quit', () => { try { fs.appendFileSync(debugLog, `[WILL_QUIT] triggered\n`); } catch(e){} });
app.on('quit', (e, code) => { try { fs.appendFileSync(debugLog, `[QUIT] exitCode=${code}\n`); } catch(e){} });
...
const rawUrl = getArg('url') || pos[0] || 'about:blank';
const width = parseInt(getArg('width') || pos[1] || '680', 10) || 680;
const height = parseInt(getArg('height') || pos[2] || '600', 10) || 600;
...
const appId = getArg('appId') || pos[3] || 'com.elix.app';
