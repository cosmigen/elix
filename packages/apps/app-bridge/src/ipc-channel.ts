...
export class ElixWindowIpcSession extends EventEmitter {
  ...
  private serviceHandlers: Map<
    string,
    (action: string, payload: any) => Promise<any> | any
  > = new Map();

  private windowActionHandler?: (action: string, params: any) => void;
  private storageData: Map<string, any> = new Map();
  private outboundTransport?: (packet: IpcPacket) => void;
   ...
   */
  public bindTransport(transport: (packet: IpcPacket) => void): void {
  ...
  public registerService(
    serviceName: string,
    handler: (action: string, payload: any) => Promise<any> | any
  ): void {
   ...
   */
  public setWindowActionHandler(handler: (action: string, params: any) => void): void {
   ...
   */
  public async handleIncomingPacket(packet: IpcPacket): Promise<void> {
    ...
    switch (packet.type) {
      ...
      case 'SERVICE_CALL': {
        const { service, action, payload } = (packet.payload as any) || {};
      ...
      case 'WINDOW_ACTION': {
        const { action, ...params } = (packet.payload as any) || {};
      ...
      case 'EVENT_EMIT': {
        const { event, payload } = (packet.payload as any) || {};
   ...
   */
  private sendPacket(packet: IpcPacket): void {
    if (this.outboundTransport) {
      try {
