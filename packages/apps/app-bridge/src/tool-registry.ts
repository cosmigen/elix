...
interface ToolRegistrationRecord {
  ...
  dispose: () => void;
  registeredTool: RegisteredAppTool;
}

/** Global map of active tool registrations: appId -> ToolRegistrationRecord[] */
const activeToolRegistrations = new Map<string, ToolRegistrationRecord[]>();

/**
 * Sanitizes an identifier segment (replaces dots, dashes, and symbols with underscores)
 */
export function sanitizeToolIdentifier(name: string): string {
 ...
 */
export function getAppToolName(appId: string, capabilityName: string): string {
...
export function registerAppTools(
  ...
  const registeredList: RegisteredAppTool[] = [];
  const records: ToolRegistrationRecord[] = [];
  ...
  for (const cap of capList) {
    ...
    const regTool: RegisteredAppTool = {
 ...
 */
export function unregisterAppTools(
  toolSinkOrCtx: ToolSink | { tools?: any; [key: string]: any },
  appId: string
): void {
 ...
 */
export function getRegisteredAppTools(appId?: string): RegisteredAppTool[] {
  if (appId) {
    const records = activeToolRegistrations.get(appId) || [];
    return records.map((r) => r.registeredTool);
  }

  const all: RegisteredAppTool[] = [];
