...
export interface FsListResult {
  ...
  drives?: DriveInfo[];
  standardFolders?: { name: string; path: string; icon: string }[];
...
export function getMimeType(filename: string): string {
  ...
  switch (ext) {
    ...
    case '.ts': case '.mts': case '.cts': return 'application/typescript';
 ...
 */
export function getAvailableDrives(): DriveInfo[] {
 ...
 */
export function getStandardUserFolders(): Array<{ name: string; path: string; icon: string }> {
 ...
 */
export async function sysFsList(dirPath?: string, includeHidden: boolean = false): Promise<FsListResult> {
 ...
 */
export async function sysFsReadWrite(params: FsReadWriteParams): Promise<FsReadWriteResult> {
 ...
 */
export async function sysHardwareScan(): Promise<HardwareScanResult> {
  const cpus = os.cpus();
  const netInterfaces = os.networkInterfaces();

  const formattedNetInterfaces: HardwareScanResult['network']['interfaces'] = [];
  ...
  const displays: HardwareScanResult['displays'] = [];
  ...
  const audioDevices: HardwareScanResult['audioDevices'] = [];
  if (siData?.audio && Array.isArray(siData.audio) && siData.audio.length > 0) {
    ...
    audioDevices.push(
      { name: 'Default System Speaker / Headphones', type: 'output', isDefault: true, status: 'active' },
      { name: 'Internal Microphone Array', type: 'input', isDefault: true, status: 'active' }
    );
  }

  const cameras: HardwareScanResult['cameras'] = [
