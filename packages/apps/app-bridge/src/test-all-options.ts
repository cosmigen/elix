...
export async function runAllOptionsAutomated(): Promise<void> {
  ...
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.notes'), 'com.elix.notes is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.calculator'), 'com.elix.calculator is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.clock'), 'com.elix.clock is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.calendar'), 'com.elix.calendar is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.utilities'), 'com.elix.utilities is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.files'), 'com.elix.files is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.media'), 'com.elix.media is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.mediastudio'), 'com.elix.mediastudio is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.studio3d'), 'com.elix.studio3d is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.documents'), 'com.elix.documents is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.capture'), 'com.elix.capture is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.camera'), 'com.elix.camera is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.terminal'), 'com.elix.terminal is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.code'), 'com.elix.code is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.sysmon'), 'com.elix.sysmon is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.devices'), 'com.elix.devices is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.settings'), 'com.elix.settings is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.appcenter'), 'com.elix.appcenter is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.search'), 'com.elix.search is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.center'), 'com.elix.center is in registry');
  assertTest(allApps.apps.some((a: any) => a.appId === 'com.elix.fakeapp'), 'com.elix.fakeapp is in registry');
  ...
  assertTest(notesInspect.capabilities.some((c: any) => c.name === 'create_note'), 'Notes declares create_note capability');
  assertTest(notesInspect.capabilities.some((c: any) => c.name === 'delete_note'), 'Notes declares delete_note capability');
  ...
  assertTest(clockInspect.capabilities.some((c: any) => c.name === 'set_alarm'), 'Clock declares set_alarm capability');
  assertTest(clockInspect.capabilities.some((c: any) => c.name === 'start_timer'), 'Clock declares start_timer capability');
  assertTest(clockInspect.capabilities.some((c: any) => c.name === 'get_world_time'), 'Clock declares get_world_time capability');
  ...
  assertTest(calInspect.capabilities.some((c: any) => c.name === 'create_calendar_event'), 'Calendar declares create_calendar_event capability');
  assertTest(calInspect.capabilities.some((c: any) => c.name === 'add_smart_task'), 'Calendar declares add_smart_task capability');
  ...
  assertTest(utilInspect.capabilities.some((c: any) => c.name === 'convert_units'), 'Utilities declares convert_units capability');
  assertTest(utilInspect.capabilities.some((c: any) => c.name === 'compute_hash'), 'Utilities declares compute_hash capability');
  assertTest(utilInspect.capabilities.some((c: any) => c.name === 'inspect_color'), 'Utilities declares inspect_color capability');
  ...
  assertTest(filesInspect.capabilities.some((c: any) => c.name === 'list_directory'), 'Files declares list_directory capability');
  assertTest(filesInspect.capabilities.some((c: any) => c.name === 'search_files'), 'Files declares search_files capability');
  assertTest(filesInspect.capabilities.some((c: any) => c.name === 'inspect_file_metadata'), 'Files declares inspect_file_metadata capability');
  ...
  assertTest(mediaInspect.capabilities.some((c: any) => c.name === 'open_media'), 'Media declares open_media capability');
  assertTest(mediaInspect.capabilities.some((c: any) => c.name === 'control_playback'), 'Media declares control_playback capability');
  assertTest(mediaInspect.capabilities.some((c: any) => c.name === 'inspect_media_info'), 'Media declares inspect_media_info capability');
  ...
  assertTest(studioInspect.capabilities.some((c: any) => c.name === 'edit_image'), 'Media Studio declares edit_image capability');
  assertTest(studioInspect.capabilities.some((c: any) => c.name === 'crop_resize'), 'Media Studio declares crop_resize capability');
  assertTest(studioInspect.capabilities.some((c: any) => c.name === 'trim_media'), 'Media Studio declares trim_media capability');
  assertTest(studioInspect.capabilities.some((c: any) => c.name === 'convert_format'), 'Media Studio declares convert_format capability');
  ...
  assertTest(studio3dInspect.capabilities.some((c: any) => c.name === 'load_model'), '3D Studio declares load_model capability');
  assertTest(studio3dInspect.capabilities.some((c: any) => c.name === 'set_viewport_shading'), '3D Studio declares set_viewport_shading capability');
  assertTest(studio3dInspect.capabilities.some((c: any) => c.name === 'inspect_mesh_stats'), '3D Studio declares inspect_mesh_stats capability');
  assertTest(studio3dInspect.capabilities.some((c: any) => c.name === 'set_environment_lighting'), '3D Studio declares set_environment_lighting capability');
  ...
  assertTest(docsInspect.capabilities.some((c: any) => c.name === 'open_document'), 'Documents declares open_document capability');
  assertTest(docsInspect.capabilities.some((c: any) => c.name === 'search_document_text'), 'Documents declares search_document_text capability');
  assertTest(docsInspect.capabilities.some((c: any) => c.name === 'get_document_outline'), 'Documents declares get_document_outline capability');
  assertTest(docsInspect.capabilities.some((c: any) => c.name === 'annotate_pdf'), 'Documents declares annotate_pdf capability');
  assertTest(docsInspect.capabilities.some((c: any) => c.name === 'edit_spreadsheet_cell'), 'Documents declares edit_spreadsheet_cell capability');
  assertTest(docsInspect.capabilities.some((c: any) => c.name === 'export_document'), 'Documents declares export_document capability');
  ...
  assertTest(captureInspect.capabilities.some((c: any) => c.name === 'capture_screen'), 'Capture declares capture_screen capability');
  assertTest(captureInspect.capabilities.some((c: any) => c.name === 'start_screen_recording'), 'Capture declares start_screen_recording capability');
  assertTest(captureInspect.capabilities.some((c: any) => c.name === 'stop_screen_recording'), 'Capture declares stop_screen_recording capability');
  assertTest(captureInspect.capabilities.some((c: any) => c.name === 'apply_annotation'), 'Capture declares apply_annotation capability');
  assertTest(captureInspect.capabilities.some((c: any) => c.name === 'get_capture_history'), 'Capture declares get_capture_history capability');
  ...
  assertTest(cameraInspect.capabilities.some((c: any) => c.name === 'take_photo'), 'Camera declares take_photo capability');
  assertTest(cameraInspect.capabilities.some((c: any) => c.name === 'start_video_recording'), 'Camera declares start_video_recording capability');
  assertTest(cameraInspect.capabilities.some((c: any) => c.name === 'stop_video_recording'), 'Camera declares stop_video_recording capability');
  assertTest(cameraInspect.capabilities.some((c: any) => c.name === 'record_audio_clip'), 'Camera declares record_audio_clip capability');
  assertTest(cameraInspect.capabilities.some((c: any) => c.name === 'list_capture_devices'), 'Camera declares list_capture_devices capability');
  assertTest(cameraInspect.capabilities.some((c: any) => c.name === 'toggle_teleprompter'), 'Camera declares toggle_teleprompter capability');
  ...
  assertTest(terminalInspect.capabilities.some((c: any) => c.name === 'spawn_shell_session'), 'Terminal declares spawn_shell_session capability');
  assertTest(terminalInspect.capabilities.some((c: any) => c.name === 'execute_command'), 'Terminal declares execute_command capability');
  assertTest(terminalInspect.capabilities.some((c: any) => c.name === 'send_terminal_input'), 'Terminal declares send_terminal_input capability');
  assertTest(terminalInspect.capabilities.some((c: any) => c.name === 'get_session_output'), 'Terminal declares get_session_output capability');
  assertTest(terminalInspect.capabilities.some((c: any) => c.name === 'kill_session'), 'Terminal declares kill_session capability');
  assertTest(terminalInspect.capabilities.some((c: any) => c.name === 'list_active_sessions'), 'Terminal declares list_active_sessions capability');
  ...
  assertTest(codeInspect.capabilities.some((c: any) => c.name === 'open_code_file'), 'Code declares open_code_file capability');
  assertTest(codeInspect.capabilities.some((c: any) => c.name === 'save_code_file'), 'Code declares save_code_file capability');
  assertTest(codeInspect.capabilities.some((c: any) => c.name === 'format_document'), 'Code declares format_document capability');
  assertTest(codeInspect.capabilities.some((c: any) => c.name === 'get_git_diff'), 'Code declares get_git_diff capability');
  assertTest(codeInspect.capabilities.some((c: any) => c.name === 'inspect_database_table'), 'Code declares inspect_database_table capability');
  assertTest(codeInspect.capabilities.some((c: any) => c.name === 'execute_sql_query'), 'Code declares execute_sql_query capability');
  ...
  assertTest(sysmonInspect.capabilities.some((c: any) => c.name === 'get_system_metrics'), 'SysMon declares get_system_metrics capability');
  assertTest(sysmonInspect.capabilities.some((c: any) => c.name === 'list_processes'), 'SysMon declares list_processes capability');
  assertTest(sysmonInspect.capabilities.some((c: any) => c.name === 'kill_process'), 'SysMon declares kill_process capability');
  assertTest(sysmonInspect.capabilities.some((c: any) => c.name === 'get_disk_io_stats'), 'SysMon declares get_disk_io_stats capability');
  assertTest(sysmonInspect.capabilities.some((c: any) => c.name === 'get_network_io_stats'), 'SysMon declares get_network_io_stats capability');
  assertTest(sysmonInspect.capabilities.some((c: any) => c.name === 'get_system_overview'), 'SysMon declares get_system_overview capability');
  ...
  assertTest(devicesInspect.capabilities.some((c: any) => c.name === 'scan_bluetooth_devices'), 'Devices declares scan_bluetooth_devices capability');
  assertTest(devicesInspect.capabilities.some((c: any) => c.name === 'toggle_bluetooth_state'), 'Devices declares toggle_bluetooth_state capability');
  assertTest(devicesInspect.capabilities.some((c: any) => c.name === 'scan_wifi_networks'), 'Devices declares scan_wifi_networks capability');
  assertTest(devicesInspect.capabilities.some((c: any) => c.name === 'configure_display'), 'Devices declares configure_display capability');
  assertTest(devicesInspect.capabilities.some((c: any) => c.name === 'set_audio_device'), 'Devices declares set_audio_device capability');
  assertTest(devicesInspect.capabilities.some((c: any) => c.name === 'get_battery_diagnostics'), 'Devices declares get_battery_diagnostics capability');
  assertTest(devicesInspect.capabilities.some((c: any) => c.name === 'list_connected_peripherals'), 'Devices declares list_connected_peripherals capability');
  ...
  assertTest(settingsInspect.capabilities.some((c: any) => c.name === 'get_system_settings'), 'Settings declares get_system_settings capability');
  assertTest(settingsInspect.capabilities.some((c: any) => c.name === 'update_setting'), 'Settings declares update_setting capability');
  assertTest(settingsInspect.capabilities.some((c: any) => c.name === 'reset_settings_category'), 'Settings declares reset_settings_category capability');
  assertTest(settingsInspect.capabilities.some((c: any) => c.name === 'manage_permissions'), 'Settings declares manage_permissions capability');
  assertTest(settingsInspect.capabilities.some((c: any) => c.name === 'clear_system_caches'), 'Settings declares clear_system_caches capability');
  assertTest(settingsInspect.capabilities.some((c: any) => c.name === 'export_configuration'), 'Settings declares export_configuration capability');
  ...
  assertTest(appcenterInspect.capabilities.some((c: any) => c.name === 'list_installed_apps'), 'App Center declares list_installed_apps capability');
  assertTest(appcenterInspect.capabilities.some((c: any) => c.name === 'search_store_catalog'), 'App Center declares search_store_catalog capability');
  assertTest(appcenterInspect.capabilities.some((c: any) => c.name === 'install_elixapp'), 'App Center declares install_elixapp capability');
  assertTest(appcenterInspect.capabilities.some((c: any) => c.name === 'uninstall_elixapp'), 'App Center declares uninstall_elixapp capability');
  assertTest(appcenterInspect.capabilities.some((c: any) => c.name === 'check_for_updates'), 'App Center declares check_for_updates capability');
  assertTest(appcenterInspect.capabilities.some((c: any) => c.name === 'inspect_package_manifest'), 'App Center declares inspect_package_manifest capability');
  ...
  assertTest(searchInspect.capabilities.some((c: any) => c.name === 'query_universal_index'), 'Search declares query_universal_index capability');
  assertTest(searchInspect.capabilities.some((c: any) => c.name === 'execute_action_shortcut'), 'Search declares execute_action_shortcut capability');
  assertTest(searchInspect.capabilities.some((c: any) => c.name === 'evaluate_quick_calculation'), 'Search declares evaluate_quick_calculation capability');
  assertTest(searchInspect.capabilities.some((c: any) => c.name === 'rebuild_search_index'), 'Search declares rebuild_search_index capability');
  assertTest(searchInspect.capabilities.some((c: any) => c.name === 'get_recent_searches'), 'Search declares get_recent_searches capability');
  ...
  assertTest(centerInspect.capabilities.some((c: any) => c.name === 'list_active_agents'), 'ELIX Center declares list_active_agents capability');
  assertTest(centerInspect.capabilities.some((c: any) => c.name === 'spawn_agent_run'), 'ELIX Center declares spawn_agent_run capability');
  assertTest(centerInspect.capabilities.some((c: any) => c.name === 'query_mcp_servers'), 'ELIX Center declares query_mcp_servers capability');
  assertTest(centerInspect.capabilities.some((c: any) => c.name === 'toggle_mcp_server'), 'ELIX Center declares toggle_mcp_server capability');
  assertTest(centerInspect.capabilities.some((c: any) => c.name === 'inspect_memory_vectors'), 'ELIX Center declares inspect_memory_vectors capability');
  assertTest(centerInspect.capabilities.some((c: any) => c.name === 'get_token_telemetry'), 'ELIX Center declares get_token_telemetry capability');
