 ...
 */
export type ElixKnownPermission =
  | 'fs:read'
  | 'fs:write'
  | 'fs:all'
  | 'net:http'
...
export interface WindowElixBridgeSystem {
  ...
  notify(title: string, options?: { body?: string; icon?: string }): Promise<void>;
}
...
export interface ElixInstallerEventMap {
  'app:file-trigger': (payload: { packagePath: string; consentPayload: AppConsentPayload }) => void;
  'app:installing': (payload: { packagePath: string; appId: string }) => void;
  'app:installed': (payload: { app: InstalledApp; isUpgrade: boolean }) => void;
  'app:uninstalling': (payload: { appId: string }) => void;
  'app:uninstalled': (payload: { appId: string }) => void;
  'error': (payload: { code: string; message: string; error?: Error }) => void;
}
...
export interface WindowManagerEvents {
  ...
  'tool:dispatch': (payload: { windowId: string; appId: string; capability: string; args: any }) => void;
  'tool:result': (payload: { windowId: string; appId: string; capability: string; result: any }) => void;
  'tool:error': (payload: { windowId: string; appId: string; capability: string; error: any }) => void;
  'error': (payload: { code: string; message: string; error?: Error }) => void;
}
