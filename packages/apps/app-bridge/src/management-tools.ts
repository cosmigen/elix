...
import { defineTool, type CoreToolDefinition } from './utils/tools.js';
...
import { sysFsList, sysFsReadWrite, sysHardwareScan, type FsReadWriteParams } from './sys-primitives.js';
...
export function createManagementTools(
  ...
  toolSinkOrCtx?: ToolSink | { tools?: any; [key: string]: any }
): CoreToolDefinition[] {
  // 1. get_app_list
  const getAppListParams: JSONSchemaObject = {
    type: 'object',
    properties: {
      filter: {
        type: 'string',
  ...
  const getAppListTool = defineTool({
    ...
    execute: async (args: { filter?: 'all' | 'running' | 'installed' }) => {
  ...
  // 2. open_app
  const openAppParams: JSONSchemaObject = {
    type: 'object',
    properties: {
      appId: {
        type: 'string',
  ...
  const openAppTool = defineTool({
    ...
    execute: async (args: { appId: string; initialRoute?: string }) => {
      if (!args || !args.appId) {
  ...
  // 3. close_app
  const closeAppParams: JSONSchemaObject = {
    type: 'object',
    properties: {
      appId: {
        type: 'string',
  ...
  // 4. install_elix_app
  const installAppParams: JSONSchemaObject = {
    type: 'object',
    properties: {
      packagePath: {
        type: 'string',
  ...
  const installAppTool = defineTool({
    ...
    execute: async (args: { packagePath: string; force?: boolean }) => {
  ...
  const uninstallAppParams: JSONSchemaObject = {
    type: 'object',
    properties: {
      appId: {
        type: 'string',
  ...
  const uninstallAppTool = defineTool({
    ...
    execute: async (args: { appId: string; keepData?: boolean }) => {
      if (!args || !args.appId) {
        throw new Error("Missing required parameter 'appId'");
      }
 ...
 * 4. sys_hardware_scan
 */
export function createSystemTools(): CoreToolDefinition[] {
  // sys_exec_code
  ...
  const sysFsListTool = defineTool({
    ...
    execute: async (args: { dirPath?: string; includeHidden?: boolean }) => {
  ...
  const sysFsReadWriteTool = defineTool({
    ...
    execute: async (args: FsReadWriteParams) => {
...
export function registerManagementTools(
  ...
  const sink: ToolSink =
    'registerTool' in toolSinkOrCtx && typeof (toolSinkOrCtx as any).registerTool === 'function'
      ? (toolSinkOrCtx as ToolSink)
      : {
          registerTool: (tool) => {
            const toolsSvc = (toolSinkOrCtx as any).tools;
