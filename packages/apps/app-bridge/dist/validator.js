...
export function validateAppManifest(input) {
    ...
    if (raw['window'] !== undefined) {
        if (typeof raw['window'] !== 'object' || raw['window'] === null || Array.isArray(raw['window'])) {
            ...
            validateWindowConfig(raw['window'], errors, warnings);
        }
    }
    ...
    const normalized = normalizeAppManifest(raw);
    return {
        valid: true,
        manifest: normalized,
        errors: [],
 ...
 */
function validateWindowConfig(win, errors, _warnings) {
    const numericProps = ['width', 'height', 'minWidth', 'minHeight', 'maxWidth', 'maxHeight'];
    ...
    const booleanProps = ['resizable', 'alwaysOnTop', 'frame', 'transparent', 'center', 'fullscreen'];
    ...
    const maxWidth = win['maxWidth'];
    const height = win['height'];
    const minHeight = win['minHeight'];
    const maxHeight = win['maxHeight'];
 ...
 */
function validateCapabilities(capabilities, errors, _warnings) {
 ...
 */
export function normalizeAppManifest(manifest) {
    ...
    return {
        ...
        permissions: Array.isArray(manifest.permissions)
            ? Array.from(new Set(manifest.permissions.map((p) => p.trim())))
            : [],
 ...
 */
export function isElixAppManifest(input) {
 ...
 */
export function assertValidAppManifest(input) {
 ...
 */
function isPathTraversalOrAbsolute(filePath) {
    if (!filePath)
        return false;
