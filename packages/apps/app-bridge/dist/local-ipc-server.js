...
export class LocalIpcServer extends EventEmitter {
    ...
    port;
    isRunning = false;
    portReclaimAttempted = false;
    constructor(options) {
        super();
        this.port = options?.port ?? 7391;
    }
    async start() {
        if (this.isRunning && this.server?.listening)
            return;
    ...
    handleIncomingPacket(packet, clientConn) {
        ...
        if (packet.jsonrpc === '2.0' || packet.method) {
            if ((packet.method === 'tools/call' && (packet.params?.name === 'sys_exec_code' || packet.params?.tool === 'sys_exec_code')) ||
                ...
                globalSysExecManager.execute(execParams, sender, (res) => {
                    if (clientConn) {
                        try {
            ...
            // sys_fs_list
            if ((packet.method === 'tools/call' && (packet.params?.name === 'sys_fs_list' || packet.params?.tool === 'sys_fs_list')) ||
            ...
            // sys_fs_read_write
            if ((packet.method === 'tools/call' && (packet.params?.name === 'sys_fs_read_write' || packet.params?.tool === 'sys_fs_read_write')) ||
            ...
            // sys_hardware_scan
            if ((packet.method === 'tools/call' && (packet.params?.name === 'sys_hardware_scan' || packet.params?.tool === 'sys_hardware_scan')) ||
    ...
    async sendToolCall(appId, capability, args, timeoutMs = 5000) {
        const set = this.clients.get(appId);
        if (!set || set.size === 0) {
...
export function getLocalIpcServer(port) {
...
//# sourceMappingURL=local-ipc-server.js.map